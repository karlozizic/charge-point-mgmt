...
import SimpleSessionPayment from '../Billing/SimpleSessionPayment';
import { formatDuration, formatEnergy } from '../../utils/format';
...
function ChargeSessionDetail() {
    ...
    const handleExportToCsv = async () => {
        try {
            await chargeSessionsApi.exportToCsv(session.id);
        } catch (error) {
    ...
    return (
        <div className="session-detail">
            ...
            <div className="session-header">
                ...
                <span className={`status ${session.status === 'Started' ? 'active' : 'completed'}`}>{session.status}</span>
                <button className="btn btn-primary" onClick={handleExportToCsv}>Export CSV</button>
            </div>
            ...
            <div className="session-content">
                <div className="session-info">
                    <div className="info-section">
                        ...
                        <div className="info-grid">
                            ...
                            <div>Duration: <strong>{formatDuration(session.startTime, session.stopTime)}</strong></div>
                        </div>
                    </div>
                ...
                {session.status === 'Stopped' && (
                    <div className="payment-section">
                        <SimpleSessionPayment sessionId={session.id} />
                    </div>
                )}
                    ...
                    <div className="meter-values">
                        ...
                        <div className="meter-table">
                            <table>
                                ...
                                <tbody>
                                {session.meterValues.map((reading, i) => (
                                    <tr key={i}>
